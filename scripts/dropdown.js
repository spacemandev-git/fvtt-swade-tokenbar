...
async function handleEvent(evt, actor) {
  ...
  if (type == "attributes") {
    ...
    if (actor.data.data.wildcard) {
      ...
      }x= +${actor.data.data.attributes[value].die.modifier} +${actor.calcWoundPenalties()} +${actor.calcStatusPenalties()} +${actor.calcFatiguePenalties()} +${parseInt(
        $("#globalMod").val()
      )}`;
    }
    ...
    if (actor.data.data.wildcard) {
      ...
      }x= +${skill.data.data.die.modifier} +${actor.calcWoundPenalties()} +${actor.calcStatusPenalties()} +${actor.calcFatiguePenalties()} +${parseInt(
        $("#globalMod").val()
      )}`;
    }
    roll(actor, `${value.toUpperCase()} Roll`, rollString);
